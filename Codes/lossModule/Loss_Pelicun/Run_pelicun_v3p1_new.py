...
from pelicun_3_1_master_new.base import convert_to_MultiIndex
from pelicun_3_1_master_new.base import convert_to_SimpleIndex
from pelicun_3_1_master_new.base import describe
from pelicun_3_1_master_new.base import EDP_to_demand_type
from pelicun_3_1_master_new.file_io import load_data
from pelicun_3_1_master_new.assessment import Assessment
...
def run_pelicun(config_path, edp_file_dir, output_fp):
    ...
    raw_demands = pd.read_csv(edp_file_dir, index_col=0)
    ...
    if out_config.get('Demand', False):
        ...
        if np.any(np.isin(['Sample', 'Statistics'], out_reqs)):
            ...
            if 'Sample' in out_reqs:
                ...
                demand_sample_s.to_csv(os.path.join(output_fp, "DEM_sample.csv"),
                                       index_label=demand_sample_s.columns.name)
            ...
            if 'Statistics' in out_reqs:
                ...
                demand_stats.to_csv(os.path.join(output_fp,"DEM_stats.csv"),
    ...
    if asset_config.get('ComponentAssignmentFile', False):
        ...
        cmp_marginals = pd.read_csv(asset_config['ComponentAssignmentFile'],
                                    index_col=1, encoding_errors='replace')
    ...
    if out_config.get('Asset', False):
        ...
        if np.any(np.isin(['Sample', 'Statistics'], out_reqs)):
            ...
            if 'Sample' in out_reqs:
                ...
                cmp_sample_s.to_csv(os.path.join(output_fp, "CMP_sample.csv"),
                                       index_label=cmp_sample_s.columns.name)

            if 'Statistics' in out_reqs:
                cmp_stats = convert_to_SimpleIndex(describe(cmp_sample), axis=1)
                cmp_stats.to_csv(
                    os.path.join(output_fp, "CMP_stats.csv"),
        ...
        if out_config.get('Damage', False):
            ...
            if np.any(np.isin(['Sample', 'Statistics',
                ...
                if 'Sample' in out_reqs:
                    ...
                    damage_sample_s.to_csv(os.path.join(output_fp, "DMG_sample.csv"),
                                       index_label=damage_sample_s.columns.name)
                ...
                if 'Statistics' in out_reqs:
                    ...
                    damage_stats.to_csv(
                        			# "DMG_stats.csv",
									os.path.join(output_fp, 'DMG_stats.csv'),
                ...
                if np.any(np.isin(['GroupedSample', 'GroupedStatistics'], out_reqs)):
                    ...
                    if 'GroupedSample' in out_reqs:
                        ...
                        grp_damage_s.to_csv(os.path.join(output_fp, 'DMG_grp.csv'),
                                            index_label=grp_damage_s.columns.name)
                    ...
                    if 'GroupedStatistics' in out_reqs:
                        ...
                        grp_stats.to_csv(
                            		# "DMG_grp_stats.csv",
                                    os.path.join(output_fp, "DMG_grp_stats.csv"),
            ...
            if out_config_loss.get('BldgRepair', False):
                ...
                if np.any(np.isin(['Sample', 'Statistics',
                    ...
                    if 'Sample' in out_reqs:
                        ...
                        repair_sample_s.to_csv(
                            os.path.join(output_fp, "DV_bldg_repair_sample.csv"),
                            index_label=repair_sample_s.columns.name)
                    ...
                    if 'Statistics' in out_reqs:
                        ...
                        repair_stats.to_csv(
                            ...
                            os.path.join(output_fp, "DV_bldg_repair_stats.csv"),
                    ...
                    if np.any(np.isin(
                        ...
                        if 'GroupedSample' in out_reqs:
                            ...
                            grp_repair_s.to_csv(
                                os.path.join(output_fp, "DV_bldg_repair_grp.csv"),
                                index_label=grp_repair_s.columns.name)
                        ...
                        if 'GroupedStatistics' in out_reqs:
                            ...
                            grp_stats.to_csv(
                                os.path.join(output_fp, "DV_bldg_repair_grp_stats.csv"),
                    ...
                    if np.any(np.isin(['AggregateSample',
                        ...
                        if 'AggregateSample' in out_reqs:
                            ...
                            agg_repair_s.to_csv(
                                os.path.join(output_fp, "DV_bldg_repair_agg.csv"),
                                index_label=agg_repair_s.columns.name)
                        ...
                        if 'AggregateStatistics' in out_reqs:
                            ...
                            agg_stats.to_csv(
                                os.path.join(output_fp, "DV_bldg_repair_agg_stats.csv"),
    ...
    # save summary sample
    summary.to_csv(
        os.path.join(output_fp, "DL_summary.csv"),
        # "DL_summary.csv", 
        index_label='#')

    # save summary statistics
    summary_stats.to_csv(
        os.path.join(output_fp, "DL_summary_stats.csv"),
...
if __name__ == '__main__':
	...
	# config_fp = os.path.join(baseDir, *['codes', 'Loss_Pelicun', 
    #                                 'pelicun_3_1_master_new', 'examples',
    #                                 '001'])
	# output_fdir = os.path.join(config_fp, 'output')
	# Path(output_fdir).mkdir(parents=True, exist_ok=True)
	# # config_fp = os.path.join(baseDir, *['codes', 'Loss_Pelicun', 
    # #                                 'pelicun_3_1_master_new', 'examples',
    # #                                 '001', 'input.json'])
	# run_pelicun(os.path.join(config_fp, 'input.json'),
    #          edp_file_dir= os.path.join(config_fp, 'demands_s4.csv'),
    #          output_fp=output_fdir)
	regional_strategy = 'EA-VaFi'
	ID = 's2_40x30_Stucco_GWB_Normal_Vs20'
	# ID = 's1_40x30_HWS_GWB_Light_Vs13'
	haz_level = 1
	baseDir = r'/Users/laxmandahal/Desktop/UCLA/Phd/Research/RegionalStudy/woodSDPA'
	config_fp = os.path.join(baseDir, *['Results',regional_strategy, ID, 'LossAnalysis','PelicunInput'])
	output_fdir = os.path.join(baseDir, *['Results',regional_strategy, ID, 'LossAnalysis','PelicunOutput', f'IL_{haz_level}'])
	Path(output_fdir).mkdir(parents=True, exist_ok=True)
	# config_fp = os.path.join(baseDir, *['codes', 'Loss_Pelicun', 
    #                                 'pelicun_3_1_master_new', 'examples',
    #                                 '001', 'input.json'])
	run_pelicun(os.path.join(config_fp, 'model_config.json'),
             edp_file_dir= os.path.join(baseDir, 'Results', regional_strategy, ID, f'demands_IL{haz_level}.csv'),
             output_fp=output_fdir)
