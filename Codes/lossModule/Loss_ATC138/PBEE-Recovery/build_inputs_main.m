...
static_data_dir = fullfile(baseDirectory, 'Codes', 'Loss_ATC138','PBEE-Recovery-atc138_prp_updates', 'static_tables');

%% PULL STATIC DATA
% Load required data tables
% component_attributes = readtable([static_data_dir filesep 'component_attributes.csv']);
component_attributes = readtable(fullfile(static_data_dir, 'component_attributes.csv'));
% damage_state_attribute_mapping = readtable([static_data_dir filesep 'damage_state_attribute_mapping.csv']);
damage_state_attribute_mapping = readtable(fullfile(static_data_dir, 'damage_state_attribute_mapping.csv'));
subsystems = readtable(fullfile(static_data_dir, 'subsystems.csv'));
tenant_function_requirements = readtable(fullfile(static_data_dir, 'tenant_function_requirements.csv'));
% fprintf('Size of component_attributes table %d\n', size(component_attributes));
%% LOAD BUILDING DATA
...
building_model = jsondecode(fileread(fullfile(loss_modelDir, 'ATC138Input', 'building_model.json')));
...
tenant_unit_list = readtable(fullfile(loss_modelDir, 'ATC138Input','tenant_unit_list.csv'), 'Format','auto');

% List of component and damage states ids associated with the damage matrix
% comp_ds_list = readtable('comp_ds_list.csv');
comp_ds_list = readtable(fullfile(loss_modelDir, 'ATC138Input',haz_level, 'comp_ds_list.csv'), 'Format','auto');

% List of component and damage states in the performance model
comp_population = readtable(fullfile(loss_modelDir, 'ATC138Input', 'comp_population.csv'));

% Read header row unaltered by matlab
fid = fopen(fullfile(loss_modelDir, 'ATC138Input', 'comp_population.csv'));
...
damage_consequences = jsondecode(fileread(fullfile(loss_modelDir, 'ATC138Input', haz_level,'damage_consequences.json')));
...
sim_damage = jsondecode(fileread(fullfile(loss_modelDir, 'ATC138Input',haz_level, 'simulated_damage.json')));
...
input_dir =  fullfile(loss_modelDir, 'ATC138Input');%, 'optional_inputs.m');
cd(input_dir)
optional_inputs
...
save(fullfile(loss_modelDir, 'ATC138Input', haz_level, 'simulated_inputs.mat'),...
